<x-layout :title="$title ?? 'Edit Post'">

    <form method="post" action="/posts/{{ $posts->id }}" class="mx-auto max-w-2xl py-10 px-4 sm:px-6 lg:px-8 color:-gray-900">
        @csrf
        @method('PUT')

        <input type="hidden" name="id" value="{{ $posts->id }}"> <!-- Include the post ID as a hidden input -->
        
        <div class="space-y-12">
            <div class="border-b border-gray-900/10 pb-12">
                <h2 class="text-base/7 font-semibold text-gray-900">Edit Post</h2>
                <p class="mt-1 text-sm/6 text-gray-600">Use this form to edit an existing post.</p>
                ...
                <div class="mt-10 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
                    ...
                    <div class="sm:col-span-3">
                        ...
                        <div class="mt-2">
                            <input id="title" type="text" value="{{ old('title', $posts->title) }}" name="title" autocomplete="given-name"
                    ...
                    <div class="sm:col-span-3">
                        ...
                        <div class="mt-2">
                            ...
                            <input id="author" type="text" value="{{ old('author', $posts->author) }}" name="author" autocomplete="family-name"
                            ...
                            <textarea
                                ...
                                class="block w-full rounded-md bg-white px-3 py-1.5 text-base text-gray-900 outline-1
                                 -outline-offset-1 placeholder:text-gray-400 focus:outline-2 focus:-outline-offset-2
                                  focus:outline-indigo-600 sm:text-sm/6 
                                  {{ $errors->has('body') ? 'outline-red-600' : 'outline-gray-300' }}">
                                  {{ old('body', $posts->body) }}</textarea>
                        </div>
